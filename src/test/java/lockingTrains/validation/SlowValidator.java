 ...
 * {@code false}.
 */
public class SlowValidator extends Recorder {
	...
	private final Location slowLocation;
	 ...
	 */
	public SlowValidator(final Problem problem, final Location slowLocation) {
		...
		this.slowLocation = slowLocation;
	...
	public synchronized void leave(final TrainSchedule schedule, final Location location) {
		if (slowLocation.id() == location.id()) {
			try {
				wait(100);
